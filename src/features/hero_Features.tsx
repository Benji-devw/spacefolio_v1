...
function StationMesh() {
  ...
  const refApesanteur = useRef<THREE.Group>(null!)  
  const scroll = useScroll()
    // GuiSettings_Basic(refGui)
  useFrame((state, delta, camera) => {
    const t = state.clock.getElapsedTime()
    ref.current.position.y =  scroll.offset * 32
    ref.current.rotation.y = - scroll.offset * 4.6
    markerRef.current.rotation.y += delta / 100
    refApesanteur.current.rotation.x = -Math.PI / 12.75 + Math.cos(t / 4) / 18
    refApesanteur.current.rotation.y = Math.sin(t / 4) / 12
    refApesanteur.current.rotation.z = (1 + Math.sin(t / 3.5)) / 20
    refApesanteur.current.position.y = (1 + Math.sin(t / 1.5)) / 20
  })

  return (
    <group ref={ref}>
      <Model name="mesh_0" position={[0, -2, 0]} rotation={[0, 0, 0]} meshColor={[8, 58, 89]} color={"#6DB4F2"}/>
      <Model name="mesh_1" position={[0, -12.5, 0]} rotation={[0, Math.PI / 2, 0]} meshColor={[87, 14, 230]} color={"#570EE6"}/>
      ...
      <Square name="SPACEFOLIO v1" position={[0, -2, 0]} >
        <group position={[0, 2.5, 0]}>
          <Component markerRef={refApesanteur} name="mesh_2" mR={0} mM={1} />
        </group>
        <Component name="mesh_3" mR={0} mM={1} position={[0, 0, 0]}/>
      ...
      <Square name="COMPETENCES" position={[0, -12.5, 0]} rotation={[0, Math.PI / 2, 0]} >
        ...
        <group name='robot'>
          <Component name="mesh_6" mR={.5}  mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_7" mR={0} mM={0} meshPos={[0, 0, 0]} />
          <Component name="mesh_8" mR={0} mM={0} meshPos={[0, 0, 0]} />
        </group>
          <Component name="mesh_9" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_10" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_11" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_12" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_13" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <Component name="mesh_14" mR={0} mM={1} meshPos={[0, 0, 0]} />
          <group ref={markerRef}>
            <Component name="mesh_15" mR={0} mM={1} meshPos={[0, 0, 0]} position={[0 , -2.7, 0]} />
          </group>
      </Square>
...
function Model({ name, position, rotation, meshColor, color, }: any) {
  const { nodes }: any = useGLTF('/Basic.glb')
  const pointLightRef = useRef(null)
  // useHelper(pointLightRef, THREE.PointLightHelper, 1, 'yellow')
  return (
    <group position={position} rotation={rotation}>
        <HaloRect rectW={3}  rectH={.5} rectI={35} boxArgs={[.1, .5, 3]}       meshColor={meshColor} color={color} position={[-3.9, 4.5, 0]} rotation={[0, 0, 0]}/>
        <HaloRect rectW={3}  rectH={.5} rectI={35} boxArgs={[.1, .5, 3]}       meshColor={meshColor} color={color} position={[0, 4.5, -3.9]} rotation={[0, -Math.PI / 2, 0]}/>
        <HaloTorus rectW={2} rectH={2}  rectI={10} boxArgs={[1.4, 0.01, 5, 8]} meshColor={meshColor} color={color} position={[0, 0.2, 0]}    rotation={[Math.PI / 2, 0, 0]}/>
        <pointLight ref={pointLightRef} castShadow position={[0, .6, 0]} color={color} distance={5} intensity={10} decay={2} shadow-mapSize={[1024, 1024]} />
        <pointLight ref={pointLightRef} castShadow position={[0, 4, -4]} color={color} distance={6} intensity={5} decay={2} shadow-mapSize={[1024, 1024]} />
        <pointLight ref={pointLightRef} castShadow position={[-4, 4, 0]} color={color} distance={6} intensity={5} decay={2} shadow-mapSize={[1024, 1024]} />
...
function Component({ refGui, markerRef, name, position, meshPos, mR, mM, ...props}: any) {
  const { nodes }: any = useGLTF('/Basic.glb')
  return (
    <group position={position}>
        <mesh ref={markerRef} position={meshPos} geometry={nodes[name].geometry}  material={nodes[name].material} castShadow receiveShadow
          {...props}/>
    </group>
  )
}
const Square = ({ name, children, ...props }: any) => {
  return (
    <group {...props}>
...
function Title({refGui, str, size, test, setColor, ...props}: any) {
...
function HaloRect({refGui, rectW, rectH, rectI, boxArgs, position, rotation, color, meshColor}: any) {
  const rectAreaLightRef = useRef(null)
  // useHelper(rectAreaLightRef, RectAreaLightHelper, '#6DB4F2' )
  return (
    <group ref={refGui} position={position} rotation={rotation}>
      <rectAreaLight ref={rectAreaLightRef} rotation={[0, -Math.PI / 2, 0]} width={rectW} height={rectH} intensity={rectI} color={color} />
...
function HaloTorus({rectW, rectH, rectI, boxArgs, position, rotation, color, meshColor}: any) {
  const rectAreaLightRef = useRef(null)
  // useHelper(rectAreaLightRef, RectAreaLightHelper, color )
  return (
    <group position={position} rotation={rotation}>
      <rectAreaLight ref={rectAreaLightRef} position={[0, 0, -2]} rotation={[Math.PI, 0, 0]} width={rectW} height={rectH} intensity={rectI} color={color} />
      <Torus args={boxArgs}>
...
function Sun({refGui, position}: any) {
  const pointLightRef = useRef(null)
